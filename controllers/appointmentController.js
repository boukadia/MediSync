...
//check role du medecin
    ...
    if (!user || user.role !== "doctor") {
      return res.status(400).json({ error: "Le médecin sélectionné est invalide" });
    }
...
// console.log({dis:disponibilite.medecin,med:doctorId});
    ...
    if (disponibilite.medecin!=doctorId) {
      return res.status(400).json({ error: "Le créneau ne correspond pas au médecin sélectionné" });
